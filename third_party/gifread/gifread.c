/*
 * gifread.c
 * A plain and simple GIF reader.
 *
 * Copyright (C) 2003-2025 Cosmin Truta and the Contributing Authors.
 *
...
/*
 * The GIF spec says that if neither global nor local color
 * maps are present, the decoder should use a system-default
 * map, which should have black and white as the first two
 * colors.
 * We previously added a few extra colors, chosen arbitrarily,
 * but stopped doing so. Missing color maps in GIF images are
 * uncommon, and other GIF readers, too, opt not to augment
 * their default maps.
 */
